...
const profileSchema = z.object({
  ...
  seniorityList: z.array(z.enum(['INTERN', 'JUNIOR', 'MID', 'SENIOR', 'SPECIALIST', 'LEAD'])).optional(),
...
const SENIORITY_OPTIONS: { value: 'INTERN' | 'JUNIOR' | 'MID' | 'SENIOR' | 'SPECIALIST' | 'LEAD'; label: string }[] = [
...
export function ProfilePage() {
  ...
  const [certificationForm, setCertificationForm] = useState({ name: '', issuer: '', issuedAt: '', expiresAt: '', credentialId: '', url: '' })
  const [showProjectForm, setShowProjectForm] = useState(false)
  const [editingProjectId, setEditingProjectId] = useState<string | null>(null)
  const [projectForm, setProjectForm] = useState({ name: '', description: '', url: '', skills: '' })
  const [showWorkExperienceForm, setShowWorkExperienceForm] = useState(false)
  const [editingWorkExperienceId, setEditingWorkExperienceId] = useState<string | null>(null)
  const [workExperienceForm, setWorkExperienceForm] = useState({ company: '', role: '', description: '', startDate: '', endDate: '', current: false, skills: '' })
  ...
  const addEducation = useMutation({
    mutationFn: (data: { degree: string; field: string; institution: string; startDate?: string | null; endDate?: string | null }) =>
  ...
  const updateEducation = useMutation({
    mutationFn: ({ id, data }: { id: string; data: { degree: string; field: string; institution: string; startDate?: string | null; endDate?: string | null } }) =>
  ...
  const addLanguage = useMutation({
    mutationFn: (data: { language: string; level: string }) =>
      profileApi.addLanguage(data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['profile'] })
  ...
  const addCertification = useMutation({
    mutationFn: (data: { name: string; issuer?: string | null; issuedAt?: string | null; expiresAt?: string | null; credentialId?: string | null; url?: string | null }) =>
      profileApi.addCertification(data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['profile'] })
      setShowCertificationForm(false)
      setCertificationForm({ name: '', issuer: '', issuedAt: '', expiresAt: '', credentialId: '', url: '' })
  ...
  const updateCertification = useMutation({
    mutationFn: ({ id, data }: { id: string; data: { name: string; issuer?: string | null; issuedAt?: string | null; expiresAt?: string | null; credentialId?: string | null; url?: string | null } }) =>
  ...
  const addProject = useMutation({
    mutationFn: (data: { name: string; description?: string | null; url?: string | null; skills?: string[] }) =>
  ...
  const updateProject = useMutation({
    mutationFn: ({ id, data }: { id: string; data: { name: string; description?: string | null; url?: string | null; skills?: string[] } }) =>
  ...
  const addWorkExperience = useMutation({
    mutationFn: (data: { company: string; role: string; description?: string | null; startDate?: string | null; endDate?: string | null; current?: boolean; skills?: string[] }) =>
      profileApi.addWorkExperience(data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['profile'] })
      setShowWorkExperienceForm(false)
      setWorkExperienceForm({ company: '', role: '', description: '', startDate: '', endDate: '', current: false, skills: '' })
  ...
  const updateWorkExperience = useMutation({
    mutationFn: ({ id, data }: { id: string; data: { company: string; role: string; description?: string | null; startDate?: string | null; endDate?: string | null; current?: boolean; skills?: string[] } }) =>
  ...
  const toggleSeniority = (value: 'INTERN' | 'JUNIOR' | 'MID' | 'SENIOR' | 'SPECIALIST' | 'LEAD') => {
  ...
  const removeStack = (stack: string) => {
    const current = watchedStacks ?? []
    profileForm.setValue('focusStacks', current.filter((s) => s !== stack))
  }
  ...
  const removeDiscardTerm = (term: string) => {
    const current = watchedDiscardTerms ?? []
    profileForm.setValue('discardTerms', current.filter((t) => t !== term))
  }
        ...
        <div className="grid gap-6">
            ...
            <CardContent>
              {editing ? (
                ...
                <div className="grid gap-3 md:grid-cols-2">
                  ...
                  <InfoRow label="Senioridade" value={profile.seniorityList?.join(', ') ?? profile.seniority ?? '-'} />
                  <InfoRow label="Localização" value={profile.location} />
                  <InfoRow
                    label="Modalidade"
                    value={REMOTE_MODE_LABELS[profile.remotePreference?.toLowerCase() as keyof typeof REMOTE_MODE_LABELS]?.label ?? profile.remotePreference}
                  />
                  <InfoRow
                    label="Salário"
                    value={
          ...
          <div className="grid gap-6 md:grid-cols-2">
            <Card>
              <CardHeader className="flex flex-row items-center justify-between">
                ...
                <Button variant="outline" size="sm" onClick={() => setShowLanguageForm(!showLanguageForm)}>
                    ...
                    <div className="grid gap-3">
                      ...
                      <div>
                        ...
                        <Select
                          ...
                          onValueChange={(value: string) => setLanguageForm({ ...languageForm, level: value })}
                        >
                    ...
                    <div key={lang.id} className="inline-flex items-center gap-1">
                      {editingLanguageId === lang.id ? (
                        <div className="inline-flex items-center gap-1">
                          ...
                          <Select
                            value={languageForm.level}
                            onValueChange={(value) => setLanguageForm({ ...languageForm, level: value })}
                          >
                          ...
                          <button
                            onClick={() => updateLanguage.mutate({ id: lang.id, data: languageForm })}
            ...
            <Card>
              <CardHeader className="flex flex-row items-center justify-between">
                ...
                <Button variant="outline" size="sm" onClick={() => setShowEducationForm(!showEducationForm)}>
              ...
              <CardContent>
                    ...
                    <div className="grid gap-3">
                      ...
                      <div className="flex gap-2">
                        <Button
                          ...
                          disabled={!educationForm.degree.trim() || !educationForm.field.trim() || !educationForm.institution.trim() || addEducation.isPending}
                        >
                ...
                <div className="space-y-2">
                  {profile.education.map((edu) => (
                    <div key={edu.id} className="text-sm bg-surface-2 rounded border border-border p-3">
                      {editingEducationId === edu.id ? (
                        <div className="grid gap-2">
                          ...
                          <div className="flex gap-2">
                            <button
                              onClick={() => updateEducation.mutate({ id: edu.id, data: educationForm })}
                              className="text-success text-sm"
                            >
                              ✓ Salvar
                            </button>
                        ...
                        >
                          ...
                          <span className="text-muted-foreground"> em {edu.field} - {edu.institution}</span>
                          <button
                            onClick={(e) => {
                              e.stopPropagation()
            ...
            <CardContent>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        ...
                        <Input
                          ...
                          onChange={(e) => setCertificationForm({ ...certificationForm, issuedAt: e.target.value })}
                          className="mt-1"
                        />
                      </div>
                      <div>
                        ...
                        <Input
                          ...
                          onChange={(e) => setCertificationForm({ ...certificationForm, expiresAt: e.target.value })}
                          className="mt-1"
                        />
                      </div>
                    </div>
              ...
              <div className="space-y-2">
                {profile.certifications?.map((cert) => (
                  <div key={cert.id} className="text-sm bg-surface-2 rounded border border-border p-3">
                    {editingCertificationId === cert.id ? (
                      <div className="grid gap-2">
                        ...
                        <div className="flex gap-2">
                          <button
                            onClick={() => updateCertification.mutate({ id: cert.id, data: certificationForm })}
                            className="text-success text-sm"
                          >
                            ✓ Salvar
                          </button>
                      ...
                      >
                        ...
                        <div className="mt-1 text-xs text-muted-foreground">
                          {cert.issuedAt && <span>Emitido em {new Date(cert.issuedAt).toLocaleDateString()}</span>}
                          {cert.expiresAt && <span className="ml-2">Vence em {new Date(cert.expiresAt).toLocaleDateString()}</span>}
                        </div>
                        {cert.url && (
                          <a href={cert.url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 text-xs text-accent hover:underline mt-1">
                ...
                {(!profile.certifications || profile.certifications.length === 0) && !showCertificationForm && (
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between">
              ...
              <Button variant="outline" size="sm" onClick={() => setShowProjectForm(!showProjectForm)}>
            ...
            <CardContent>
                  ...
                  <div className="grid gap-3">
                    ...
                    <Button
                      ...
                      onClick={() => addProject.mutate({
                        ...
                        skills: projectForm.skills ? projectForm.skills.split(',').map(s => s.trim()).filter(Boolean) : [],
                      })}
              ...
              <div className="space-y-2">
                {profile.projects?.map((proj) => (
                  <div key={proj.id} className="text-sm bg-surface-2 rounded border border-border p-3">
                    {editingProjectId === proj.id ? (
                      <div className="grid gap-2">
                        ...
                        <div className="flex gap-2">
                          <button
                            onClick={() => updateProject.mutate({ id: proj.id, data: { ...projectForm, skills: projectForm.skills ? projectForm.skills.split(',').map(s => s.trim()).filter(Boolean) : [] } })}
                            className="text-success text-sm"
                          >
                            ✓ Salvar
                          </button>
                      ...
                      >
                        <div className="flex items-center justify-between">
                          <div>
                            ...
                            {proj.url && (
                              <a href={proj.url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 text-xs text-accent hover:underline ml-2">
                                <ExternalLink className="h-3 w-3" />
                                Link
                              </a>
                            )}
                        ...
                        {proj.skills?.length > 0 && (
                          <div className="mt-2 flex flex-wrap gap-1">
                            {proj.skills.map((skill) => (
                              <span key={skill} className="inline-flex items-center rounded-full bg-surface px-2 py-0.5 text-xs text-foreground">
                                {skill}
                              </span>
                            ))}
                          </div>
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between">
              ...
              <Button variant="outline" size="sm" onClick={() => setShowWorkExperienceForm(!showWorkExperienceForm)}>
            ...
            <CardContent>
                  ...
                  <div className="grid gap-3">
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        ...
                        <Input
                          ...
                          onChange={(e) => setWorkExperienceForm({ ...workExperienceForm, company: e.target.value })}
                      ...
                      <div>
                        ...
                        <Input
                          ...
                          onChange={(e) => setWorkExperienceForm({ ...workExperienceForm, role: e.target.value })}
                    ...
                    <div>
                      ...
                      <textarea
                        ...
                        onChange={(e) => setWorkExperienceForm({ ...workExperienceForm, description: e.target.value })}
                        rows={3}
                        className="flex w-full rounded-[var(--radius-sm)] border border-border bg-surface px-3 py-2 text-sm text-foreground placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent focus-visible:ring-offset-2 mt-1"
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        ...
                        <Input
                          ...
                          onChange={(e) => setWorkExperienceForm({ ...workExperienceForm, startDate: e.target.value })}
                          className="mt-1"
                        />
                      </div>
                      <div>
                        ...
                        <Input
                          ...
                          onChange={(e) => setWorkExperienceForm({ ...workExperienceForm, endDate: e.target.value })}
                    ...
                    <div className="flex items-center gap-2">
                      <input
                        ...
                        onChange={(e) => setWorkExperienceForm({ ...workExperienceForm, current: e.target.checked })}
                        className="rounded border-border"
                      />
                      <label htmlFor="current-work" className="text-sm font-medium">Trabalho atual</label>
                    </div>
                    <div>
                      <label className="text-sm font-medium">Skills (separadas por vírgula)</label>
                      <Input
                        value={workExperienceForm.skills}
                        onChange={(e) => setWorkExperienceForm({ ...workExperienceForm, skills: e.target.value })}
                    ...
                    <Button
                      size="sm"
                      onClick={() => addWorkExperience.mutate({
                        ...
                        endDate: workExperienceForm.current ? null : workExperienceForm.endDate || null,
                        current: workExperienceForm.current,
                        skills: workExperienceForm.skills ? workExperienceForm.skills.split(',').map(s => s.trim()).filter(Boolean) : [],
                      })}
                      disabled={!workExperienceForm.company.trim() || !workExperienceForm.role.trim() || addWorkExperience.isPending}
                    >
              ...
              <div className="space-y-2">
                {profile.workExperiences?.map((exp) => (
                  <div key={exp.id} className="text-sm bg-surface-2 rounded border border-border p-3">
                    {editingWorkExperienceId === exp.id ? (
                      <div className="grid gap-2">
                        ...
                        <div className="flex gap-2">
                          <button
                            onClick={() => updateWorkExperience.mutate({ id: exp.id, data: { ...workExperienceForm, skills: workExperienceForm.skills ? workExperienceForm.skills.split(',').map(s => s.trim()).filter(Boolean) : [] } })}
                            className="text-success text-sm"
                          >
                            ✓ Salvar
                          </button>
                      ...
                      >
                        ...
                        <div className="mt-1 text-xs text-muted-foreground">
                          {exp.startDate && <span>Início: {new Date(exp.startDate).toLocaleDateString()}</span>}
                          {!exp.current && exp.endDate && <span className="ml-2">Término: {new Date(exp.endDate).toLocaleDateString()}</span>}
                ...
                {(!profile.workExperiences || profile.workExperiences.length === 0) && !showWorkExperienceForm && (
                  <span className="text-muted-foreground">Nenhuma experiência profissional adicionada</span>
                )}
              </div>
            </CardContent>
          </Card>
...
function ProfileForm({ form, onSubmit, seniorityOptions, jobTypeOptions, watchedSeniority, watchedJobTypes, watchedStacks, watchedDiscardTerms, toggleSeniority, toggleJobType, newStack, setNewStack, addStack, removeStack, newDiscardTerm, setNewDiscardTerm, addDiscardTerm, removeDiscardTerm, isPending, onCancel }: {
  form: any
  ...
  return (
    ...
    >
      ...
      <div>
        <label className="text-sm font-medium mb-2 block">Senioridade (selecione uma ou mais)</label>
        <div className="flex flex-wrap gap-2">
      ...
      <div>
        <label className="text-sm font-medium mb-2 block">Tipos de cargo (selecione um ou mais)</label>
        <div className="flex flex-wrap gap-2">
      ...
      <div>
        ...
        <div className="flex gap-2 mb-2">
          ...
          />
          <Button type="button" onClick={addStack} variant="outline">+</Button>
        </div>
        <div className="flex flex-wrap gap-2">
          {watchedStacks?.map((stack: string) => (
            <span key={stack} className="inline-flex items-center gap-1 rounded-full bg-success/10 text-success px-3 py-1 text-sm">
              {stack}
              <button type="button" onClick={() => removeStack(stack)} className="hover:text-error">×</button>
            </span>
          ))}
        </div>
      </div>
      ...
      <div>
        ...
        <div className="flex gap-2 mb-2">
          ...
          />
          <Button type="button" onClick={addDiscardTerm} variant="outline">+</Button>
        </div>
        <div className="flex flex-wrap gap-2">
          {watchedDiscardTerms?.map((term: string) => (
            <span key={term} className="inline-flex items-center gap-1 rounded-full bg-error/10 text-error px-3 py-1 text-sm">
              {term}
              <button type="button" onClick={() => removeDiscardTerm(term)} className="hover:text-error/60">×</button>
            </span>
          ))}
        </div>
      </div>
